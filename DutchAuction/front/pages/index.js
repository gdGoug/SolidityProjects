...
const ERROR_CORE_TX_REJECTED_BY_USER = 4001
...
export default class extends Component {
    ...
    async _initialize(selectedAddress){
        ...
        this.startingPrice = await this._auction.getStartingPrice(0)
        this.startAt = await this._auction.getStartAt(0)
        this.discountRate = await this._auction.getDiscountRate(0)
    ...
    updateStopped = async() => {
        const stopped = await this._auction.getStopped(0)
    ...
    componentWillUnmount(){
        clearImmediate(this.checkPriceInterval)
    }
    ...
    buy = async() =>{
        try{
            const tx = await this._auction.buy(0,{
            ...
            if(error.code === ERROR_CORE_TX_REJECTED_BY_USER) {return}
