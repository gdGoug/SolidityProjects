...
contract Governance{
    ...
    IERC20 public  token;
    uint public constant VOTING_DURATION = 120;
    ...
    function propose(address _to, uint _value, string calldata _func, bytes calldata _data, string calldata _description) external returns(bytes32) {
        ...
        bytes32 proposalId = generateProrposalId(_to, _value, _func, _data, keccak256(bytes(_description)));

        require(proposals[proposalId].votingStarts == 0, "proposal alredy exists");
    ...
    function execute( address _to, uint _value, string calldata _func, bytes calldata  _data, bytes32 _descriptionHash) external returns(bytes memory){
        bytes32 proposalId = generateProrposalId(_to, _value, _func, _data, _descriptionHash);
    ...
    function vote(bytes32 proposalId, uint8 voteType) external {
        ...
        require(!proposalVote.hasVoted[msg.sender], "alredy voted");
    ...
    function generateProrposalId(address _to, uint _value, string calldata _func, bytes calldata _data, bytes32 _descriptionHash) internal pure returns(bytes32){
